...
import { CircularProgress, Stack, Tooltip, Typography, MenuItem, Select, Input, TextField, LinearProgress, Skeleton, Button, FormGroup, FormControlLabel, Switch, Box, InputAdornment } from '@mui/material';
...
const SHIPMENT_TYPE = 'teamLeader';

export default function TeamLeaderScreenTable() {
    ...
    const handleSaveClick = async (row) => {
        let { unloading_start_time, unloading_end_time } = selectedRow;

        if (row.status === 'READY_FOR_UNLOADING') {
            if (!fieldValidation({ unloading_start_time }, displayName)) {
                return;
            }
        } else if (row.status === 'UNLOADING_START') {
            if (!fieldValidation({ unloading_end_time }, displayName)) {
                return;
            }
        }
        ...
        try {
            ...
            if (row.status === 'READY_FOR_UNLOADING') {
                updateData.update_type = 'US'
                updateData.unloading_start_time = unloading_start_time.utc().valueOf() // converting to timestamp
                updateData.status = 'UNLOADING_START'
            } else if (row.status === 'UNLOADING_START') {
                updateData.update_type = 'CE'
                updateData.unloading_end_time = unloading_end_time.utc().valueOf() // converting to timestamp
                updateData.status = 'CONTAINER_EMPTY'
            }
    ...
    const handleEditClick = (row) => {
        if (!row.unloading_start_time) {
            setSelectedRow({ ...row, unloading_start_time: dayjs(new Date())});
        } else {
            setSelectedRow({ ...row, unloading_end_time: dayjs(new Date())});
        }
    };
                                ...
                                .map((row, rowIndex) => (
                                    ...
                                    >
                                        {displayAttribute.map((column, columnIndex) => {
                                            if (((column === 'unloading_start_time' && row.status === 'READY_FOR_UNLOADING') || (column === 'unloading_end_time' && row.status === 'UNLOADING_START')) && isRowSelected(row.PK)) {
                                                return (<TableCell 
