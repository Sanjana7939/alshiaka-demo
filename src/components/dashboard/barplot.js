...
const palette = [
  AppTheme["NEW"],
  AppTheme["PLACEMENT_PROVIDED"],
  AppTheme["READY_FOR_UNLOADING"],
  AppTheme["UNLOADING_START"],
  AppTheme["CONTAINER_EMPTY"],
  AppTheme["CONTAINER_LEFT_DC"],
];
  ...
  return (
    <div
      style={{
        ...
        marginTop: "px",/////////////////////////////////check if any changes made by mistake
        marginRight: isXs ? '5px' : '20px',//////////////////check if any changes made by mistake
        paddingBottom: "5px",
      }}
    >
      {legendData.map((status, index) => (
        ...
        >
          <div
            style={{
              width: isXs ? "15px" : "20px",
              height: isXs ? "15px" : "20px",
...
export default function BasicBars({ data }) {
  ...
  const legendData = [
    ...new Set(seriesData.flatMap((series) => series.status)),
  ];
