...
const FILTER_COLUMNS = ['container_id', 'mode', 'status', 'asn_no', 'facility_id', 'location']
const SHIPMENT_TYPE = 'security';

export default function SecurityScreenTable() {
    ...
    const handleSaveClick = async (row) => {
        let { dc_entry_time, dc_exit_time, dock } = selectedRow;

        if (row.status === 'PLACEMENT_PROVIDED') {
            if (!fieldValidation({ dc_entry_time, dock }, displayName)) {
                return;
            }
        } else if (row.status === 'CONTAINER_EMPTY') {
            if (!fieldValidation({ dc_exit_time }, displayName)) {
                return;
            }
        }
        ...
        try {
            ...
            if (row.status === 'PLACEMENT_PROVIDED') {
                updateData.update_type = 'RFU'
                updateData.dc_entry_time = dc_entry_time.utc().valueOf() // converting to timestamp
                updateData.status = 'READY_FOR_UNLOADING',
                updateData.dock = dock.entityID
            } else if (row.status === 'CONTAINER_EMPTY') {
                updateData.update_type = 'CLD'
                updateData.dc_exit_time = dc_exit_time.utc().valueOf() // converting to timestamp
                updateData.status = 'CONTAINER_LEFT_DC'
            }
    ...
    const handleEditClick = (row) => {
        if (!row.dc_entry_time) {
            setSelectedRow({ ...row, dc_entry_time: dayjs(new Date())});
        } else {
            setSelectedRow({ ...row, dc_exit_time: dayjs(new Date())});
        }
    };
    ...
    const getDockData = (column, dockEntityId) => {
        if (typeof selectedRow[column] === 'object') {
            return selectedRow[column]
        }

        dockList.forEach((dock) => {
            if (dock.entityID === dockEntityId) {
                handleChange(column, dock)
                return selectedRow[column]
            }
        })
                                ...
                                .map((row, rowIndex) => (
                                    ...
                                    >
                                        {displayAttribute.map((column, columnIndex) => {
                                            if (column === 'dock' && row.status === 'PLACEMENT_PROVIDED' && isRowSelected(row.PK)) {
                                                return (<TableCell 
                                                ...
                                                >
                                                    <Autocomplete
                                                        sx={{ width: '20ch' }}
                                                        value={getDockData(column, row[column])}
                                            ...
                                            if (((column === 'dc_entry_time' && row.status === 'PLACEMENT_PROVIDED') || (column === 'dc_exit_time' && row.status === 'CONTAINER_EMPTY')) && isRowSelected(row.PK)) {
                                                return (<TableCell 
                                                    key={`cell-${columnIndex}`}
                                                    sx={{ height: '0px', fontSize: responsiveFontSize }}
                                                    style={{ ...getColumnStyles(column) }}
                                            ...
                                            if (column === 'facility_id' || column === 'location' || column === 'dock') {
                                                return (<TableCell 
                                                    key={`cell-${columnIndex}`}
                                                    sx={{ height: '0px', fontSize: responsiveFontSize }}
                                                    style={{ 
