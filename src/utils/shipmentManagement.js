  ...
  const data = containersList.filter((row) => {
    ...
    for (let key in filterHeaderData) {
      ...
      if (key === "status") {
        if (
          row[key] &&
          !statusAttributes[row[key]].description
            .toString()
            .toLowerCase()
            .startsWith(filterHeaderData[key].toString().toLowerCase())
        ) {
          flag = false;
        }
      } else if (
        ...
        if (
          row[key] &&
          !getEntityDesc(key, row[key], dcList, dockList, facilityList)
            .toLowerCase()
            .startsWith(filterHeaderData[key].toString().toLowerCase())
        ) {
          flag = false;
